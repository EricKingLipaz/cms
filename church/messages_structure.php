...
<html>
<head>
    <title>Messages Table Structure</title>
    <script src='https://cdn.tailwindcss.com'></script>
</head>
<body class='bg-gray-100'>
    <div class='max-w-4xl mx-auto p-6'>
        <h1 class='text-3xl font-bold text-center text-gray-800 mb-8'>Messages Table Structure</h1>";

try {
    // Check if messages table exists
    $stmt = $dbh->prepare("SHOW TABLES LIKE 'messages'");
    $stmt->execute();
    
    if ($stmt->rowCount() > 0) {
        // Check structure of messages table
        $stmt = $dbh->prepare("DESCRIBE messages");
        $stmt->execute();
        $columns = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        echo "<div class='bg-white shadow rounded-lg p-6 mb-6'>
                <h2 class='text-xl font-bold text-gray-800 mb-4'>messages Table Structure</h2>
        ...
        echo "<div class='bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative mb-6'>
                <strong>Warning:</strong> messages table does not exist
              </div>";
    }
